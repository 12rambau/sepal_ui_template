...
class DefaultVizTile(sw.Tile):
    ...
    def _on_run(self, widget, data, event): 
        ...
        if not self.alert.check_input(self.aoi_model.name, cm.default_process.no_aoi): return 
        if not self.alert.check_input(self.model.slider_value, cm.default_process.no_slider): return 
        if not self.alert.check_input(self.model.text_value, cm.default_process.no_textfield): return 
