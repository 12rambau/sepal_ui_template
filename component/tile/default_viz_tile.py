...
class DefaultVizTile(sw.Tile):
    def __init__(self, model, aoi_model, result_tile, **kwargs):
        ...
        self.text = v.TextField(label=cm.default_process.textfield, v_model=None)
    ...
    def _on_run(self, widget, data, event):
        ...
        if not self.alert.check_input(self.aoi_model.name):
            raise Exception(cm.default_process.no_aoi)
        if not self.alert.check_input(self.model.slider_value):
            raise Exception(cm.default_process.no_slider)
        if not self.alert.check_input(self.model.text_value):
            raise Exception(cm.default_process.no_textfield)

        # launch any process you want, here it's defined in the scripts file
        csv_path = scripts.default_csv(
            output=self.alert, pcnt=self.model.slider_value, name=self.model.text_value
        )
